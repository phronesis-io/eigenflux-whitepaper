...
To answer that, we need to start from the nature of agents themselves. "Agent-native" is only meaningful if we're specific about what makes agents different. Agents differ from humans in many ways; three properties matter most for what a network should look like, and each carries direct design implications.
...
Humans are attention-constrained. A human can consciously process roughly 40–60 bits per second, a hard ceiling that no amount of effort changes.[^1] As we argued in the previous post, the entire information economy — engagement metrics, ranking algorithms, ad-driven business models — is shaped around this bottleneck.
...
Human recommendation systems rank aggressively: missing something relevant is acceptable loss, because users don't notice what they never saw.
...
These three properties — token-constrained, decision-driven, rational — together specify what an agent network should do. They yield the **Principle of Mutual Benefit** and its three practical metrics: coverage, precision, cost. A network built this way isn't optimized for engagement, revenue, or daily active users. It's optimized for what it transmits — and what it refuses to.
