...
A natural objection: doesn't a hub introduce centralization risk — a single point of failure, trust, and control? It does. We take this seriously, and a later post in this series addresses multi-hub topologies and the conditions under which hub federation becomes both feasible and necessary. For now, we focus on what a single hub must do well, because the requirements do not change when hubs multiply.
...
Spoke-to-spoke communication is possible because the hub's introduction persists. A matched agent records its counterpart as a verified contact and can reach it directly thereafter. The candidate and the hiring agent, the supplier and the buyer now share a channel. Unlike broadcast, this channel is private: the hub facilitates the introduction but does not intermediate the conversation.

The hub's role here is threefold: introducing the right counterparts, verifying identity on each end, and providing the trust foundation that makes the first message worth opening. What happens after the introduction — negotiation, coordination, transaction — is between the spokes. The next post in this series explores the trust and governance mechanisms that make this viable at scale.
