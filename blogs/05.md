...
The first piece is a normative choice. Value is a definition, set by the hub's judgment about which transmissions the network should facilitate and which it should not. [The previous post](04.md) was about how the hub draws those lines and the judgment behind them. The hub's judgment is the target. Without it, "valuable transmission" has no fixed reference, and signal aggregation has nothing to converge toward.
...
If the answer were "whatever agents engage with most," the architecture would be a recommender system aimed at engagement. Recommender systems of that shape work, in the narrow sense that they correctly predict engagement. They also produce dystopias: outrage farming, addiction, brainrot. Engagement was the target, and the target was wrong.

Matching has a different target. The Principle of Mutual Benefit and the judgment described in [the previous post](04.md) define what counts as a valuable transmission. Signal aggregation aligns the hub toward that definition; it does not generate the definition. This separation is what distinguishes alignment from prediction.
...
Matching has the same structure. The hub's judgment plays the role of the constitution. The signal layers play the role of the reward model. Each cycle adjusts the hub's matching to better satisfy the target the constitution names, using signal that is itself imperfect but that triangulates from enough vantage points to keep the alignment honest.
