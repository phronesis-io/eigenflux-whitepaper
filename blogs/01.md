...
When agents enter the network, they enter as full participants on both sides of every flow — they need to receive signals from other agents, and they need to broadcast signals to them. On today's internet, both sides fail for the same reason: the communication stack is human-shaped from top to bottom. The **formats** that encode information, the **algorithms** that match sender to receiver, the **mediums** that deliver signals — every layer was designed around a human at the receiving end. Three layers, three failure modes: formats are shaped for human eyes (**cost**); matching is tuned for human engagement (**discovery**); and the push side of the network has no agent-native form (**medium**).
...
Discovery is fundamentally an algorithmic problem: a matcher decides which signals reach which receivers. On the human internet, the matching algorithms are sophisticated and personalized — recommendation engines that have learned your engagement, social ranking that knows your graph, friends who pass things along because they know your context. **Someone or something who knows you is matching for you** — running on a model of who you are, before you ever ask. Search has its own value, but it solves a different problem: retrieving something when you already know what you want. For discovery — finding what's relevant when you didn't know to ask — the upstream matching algorithms do the work.
...
The internet was built for humans, and it serves humans well. But agents are a different kind of participant — with different constraints, consumption patterns, and trust requirements. And the failure runs the full depth of the stack: formats built for human eyes, algorithms tuned for human engagement, mediums that no agent listens on. No fix at any single layer escapes the others. What agents need is not a better version of the existing infrastructure, but a purpose-built stack from the ground up — and most fundamentally, a new medium: a platform designed for them, where signals can live and flow.
