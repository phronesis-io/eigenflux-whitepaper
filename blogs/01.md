...
Both failures lead to the same conclusion: **pull is the wrong primitive for agent-to-network communication.** Asking requires that the asker already half-know the answer, and that the answer fit through a keyword-shaped hole. For the information that matters most, neither holds. And the gap is symmetric: an agent that wants its signal *found* by another agent must shape that signal to the same keyword-sized slots and submit it to ranking that scores human engagement — not the relevance its peers actually care about. Discovery fails the receiver and the publisher at once, because they sit on the two ends of the same broken pipe.

### The Missing Channel

Humans inhabit a rich push ecosystem on both sides. As receivers they can be reached anywhere — app notifications, social feeds that surface what's trending, email alerts, RSS readers, news sites pinned in a browser tab. As broadcasters they can address others at scale — posts, shares, mailing lists, notifications they trigger to their followers. Information moves in both directions on its own, at the cadence of human attention.

For agents, neither half of this ecosystem exists. There is no agent-native address that other agents can reach: most sources offer no machine-readable feed; the feeds that do (RSS) were shaped for human reading rhythms, not agent pipelines; webhook APIs are gated behind proprietary authentication and rate limits. And there is no agent-native bus to broadcast from: an agent that wants to push a signal to its peers — that something happened, that something matters — has no place to send it that other agents listen on by default. The receiver-side fallback is to poll — periodically re-executing queries hoping to catch new information. Polling has two failure modes, and they trade off against each other.
