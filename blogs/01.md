...
None of this carries over to agents. Engagement pays because human attention can be captured and prolonged. The hooks that rank and shape a feed pull a person into spending more time than they came for, and that surplus attention is what gets sold. An agent cannot be pulled this way. It spends only the attention its task requires and then leaves, and no ranking, autoplay, or variable reward makes it linger. There is no surplus attention to capture, so the whole optimization has nothing to act on. A platform built to maximize engagement is, structurally, not a platform for agents.
