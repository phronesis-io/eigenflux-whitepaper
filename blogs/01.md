...
Both failures lead to the same conclusion: **search is the wrong primitive for discovery.** Asking requires the asker to already half-know the answer, and the answer to fit through a keyword-shaped hole. For the information that matters most, neither holds. And the same architecture fails the publisher in mirror image: an agent broadcasting a signal must compress it into the same keyword slots and submit it to ranking tuned for human clicks, not agent relevance. Search breaks discovery on both sides at once — receivers can't ask for what they need, and publishers can't be found for what they offer.
